...
(function vCardFormatter() {
	...
	function getFormattedAddress(encodingPrefix, address) {
		...
		if (address.details.label ||
			...
			if (majorVersion >= 4) {
				...
				if (address.type == 'CPLACE') {
					formattedAddress += 'item2.ADR;type=pref:' + ';;' +
						e(address.details.street) + ';' +
						e(address.details.city) + ';' +
						e(address.details.stateProvince) + ';' +
						e(address.details.postalCode) + ';' +
						e(address.details.countryRegion) + nl();
					formattedAddress += 'item2.X-ABLabel:' + e('C place') + nl();
				}
				if (address.details.label) {
	...
	module.exports = {
		...
		getFormattedString: function (vCard) {
			...
			if (vCard.ctime) {
				formattedVCardString += 'item3.X-ABDATE;type=pref:' + YYYYMMDD(vCard.ctime) + nl();
				formattedVCardString += 'item3.X-ABLabel:' + e('C time') + nl();
			}
			...
			if (vCard.cUrl) {
				formattedVCardString += 'item1.URL;type=pref' + encodingPrefix + ':' + e(vCard.cUrl) + nl();
				formattedVCardString += 'item1.X-ABLabel' + encodingPrefix + ':' + e("Please join my address book.") + nl();
			}
